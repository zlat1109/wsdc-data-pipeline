...
**Merge keys are retired ids.** Once a duplicate row is merged and dropped, the registry max id shrinks below it, and the next full parse would mint the same number for an unrelated new city — `consolidate_location_ids` then remapped it onto the merge target (398 → São Paulo 243, 401 → St. Petersburg 222, 412 → Brno 266, 423 → Perth, 435 → Wailea, …). That mechanism produced every "shared wrong location_id" incident. `resolve_result_location_ids` now skips all merge-map keys **and** `LOCATION_ID_CORRECTIONS` keys (`retired_location_ids()`) when allocating fresh ids — the corrections map is id-keyed too and `export.py` re-applies it, so a re-minted key would inherit another city's coordinates. `tests/test_resolve_retired_ids_and_region_fallback.py` guards both, and asserts that every corrections key still has a live row in `data/location_info.csv` while no merge key does.

**Three-part WSDC strings.** The lookup API emits `City, Region, Country` (`Incheon, Incheon, South Korea`, `Helsinki, Uusimaa, Finland`) while the registry stores `City, Country`. `resolve_result_location_ids` falls back to a city+country key (`city_country_fallback_key`) for non-US strings before inventing a new row, so known cities no longer spawn duplicates that need merge-map entries.
