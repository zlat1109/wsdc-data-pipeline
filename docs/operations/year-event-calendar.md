...
- **Provisional unlinked trials**: confirmed Trial rows in `as_of.year` with no `event_id` that have not ended yet are projected as `expected` **Registry** into later selector years (trial is first-year only). Series key = name fingerprint (stopwords stripped, trailing city dropped when known) + country. The bridge stops once the source edition has passed **or** a normal catalog `event_id` appears — there can be a short gap after the weekend until the next scrape links the series.
