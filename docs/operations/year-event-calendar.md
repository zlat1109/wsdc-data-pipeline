...
- **Provisional unlinked rows**: confirmed list/schedule rows with no `event_id` and year `< target` are projected as `expected` **Registry** into later selector years (trial is first-year only). Priors can be upcoming current-year editions **or** already-published future/past list years (e.g. a 2027 list-only row → 2028 expected). Series key = name fingerprint (stopwords + edition-year tokens stripped, trailing city dropped when known) + country. For `as_of.year` the bridge stops once the edition has passed without a catalog id — there can be a short gap until the next scrape links the series.
