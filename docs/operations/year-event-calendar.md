...
- **Provisional unlinked trials**: confirmed Trial rows in `as_of.year` with no `event_id` that have not ended yet are projected as `expected` (still Trial) into later selector years. This bridge stops once the edition has passed or a normal catalog `event_id` appears.
