...
- Missing `location_id` on schedule rows inherits the latest edition `location_id` when the published city is missing or still aliases that place (Tampa Bay↔Tampa); then `location_info` coords. Real list relocations (different city vs last edition) keep the schedule city/country and resolve coords via city/country fallback — old edition lids must not overwrite the live list.
- Flat `EVENT_NAME_LOCATION_OVERRIDES` still correct wrong shared lids on edition/result rows. On live list rows (`scheduled_events` / `events_list_current`, and YoY expected stubs projected from them), an incompatible published city wins over a flat override (e.g. DCSX Washington vs Herndon). Year-scoped `EVENT_NAME_YEAR_LOCATION_OVERRIDES` always apply (documented venue moves such as GGP→Paris, Countdown→Mansfield). Soft city match folds accents and `St.`/`Saint` and uses token-prefix aliases (Tampa↔Tampa Bay), not bare substrings.
...
- **Unlinked ended retention**: WSDC calendar editions that never matched a catalog `event_id` (`edition_date_matches.match_status=unmatched`) stay on the year calendar as `confirmed` with `provisional_unlinked` for **`UNLINKED_ENDED_RETENTION_DAYS` (7)** after `end_date`. Covers the gap when the live events list drops a finished Trial (e.g. Manneken Swing) before points/catalog catch up. Geo/kind restored from list changelog / schedule via `source_fingerprint`. No numeric provisional `event_id` (avoids YoY ghosts); public `id` uses the existing `t-<slug>` token. Drops when grace expires or a catalog match appears.
...
- **Operator overrides**: curated gaps (not yet on WSDC calendar/list, or scrape mis-matched) live in `transform/knowledge/calendar_operator_overrides.py`. They are upserted after each calendar sync with `date_source=operator` **and** read directly by the year-calendar builder. Official `wsdc_calendar` for the same `(event_id, year, month)` wins in DB **except** it must not replace day-precision with a month stub, and must not change past day-precision rows (see upsert WHERE in `db/edition_calendar.py`). Examples: Indy Dance Explosion 2025 day-dates from the event site (`255`, month-only WSDC stub); Dance Mardi Gras 2026 provisional hiatus (`148`); Soul Flow 2026 hiatus + 2027 expected under provisional id `990001` (WSDC lists Soul Flow under the old Toulouse GGP URL — do not merge into `342`).
- **Dump backfill (one-shot)**: `scripts/sync_dump_edition_dates.py` fills month stubs from local clone `competitionevents` (`date_source=wsdc_dump`). Not part of every parse; match on series `event_id` + MERGE map.
...
- **Missing day dates** with results: emit `stats_only` from `event_editions` (placeholder `edition_date` copied onto `start_date`/`end_date` with `date_source=edition` when dump/calendar has no day range); exclude from YoY expected priors; site skips day cells. Drop a `stats_only` row when the same `event_id`+year already has a day-precision calendar/schedule row (month placeholder vs real weekend).
