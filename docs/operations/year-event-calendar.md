...
- **Series successor suppression**: for known rebrands that intentionally keep separate registry ids (e.g. `264` Swedish Swing Summer Camp -> `493` UpTown Swing), a terminal/published row in target year suppresses expected on linked legacy id to avoid duplicate old-name ghosts.
