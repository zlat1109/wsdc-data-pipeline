...
# Канонические координаты города для карт: пригороды/площадки → центр города.
# Ключ: (event_city, event_country, event_state или '').
...
def normalize_geography(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 3. Канонические координаты города (пригороды/площадки → один маркер на карте)
    df = canonicalize_city_coordinates(df)

    # 4. Стандартизация локаций
