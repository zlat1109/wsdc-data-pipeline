...
def test_merge_suppresses_and_purges_held_point_summaries():
    """Withheld editions must not be created and must be stripped if already published."""
    existing = {
        "summaries": [
            {
                "post_date": "15-09-2026",
                "events_count": 1,
                "events": [
                    {
                        "slug": "2026-09-10-bavarian-open",
                        "name": "Bavarian Open",
                        "divisions": [{"division": "All-Star", "places": []}],
                        "start_date": "2026-09-10",
                        "end_date": "2026-09-14",
                    }
                ],
    ...
    assert "2026-09-10-bavarian-open" in report["skipped"]
    assert report["purged"] == ["2026-09-10-bavarian-open"]
    events = [e for b in payload["summaries"] for e in b["events"]]
    assert [e["slug"] for e in events] == ["2026-09-10-swingtime-in-the-rockies"]
